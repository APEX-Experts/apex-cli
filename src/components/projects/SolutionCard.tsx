...
export function SolutionCard({ supertitle, title, items }: SolutionCardProps) {
  ...
  return (
    <div
      ref={ref}
      {...glowProps}
      className="group relative rounded-2xl border border-white/20 p-8 md:p-10 overflow-hidden transition-all duration-500 hover:border-sinai-glow-orange/30 bg-sinai-glow-orange/3 backdrop-blur-xs"
    >
          ...
          {items.map((item, index) => (
            <li key={index} className="flex flex-col gap-1.5">
              <div className="flex flex-row gap-3 items-center">
                <CheckCircle2 className="w-4 h-4 text-sinai-glow-orange shrink-0" />
              ...
              <p className="pl-7 text-sm text-white/40 leading-[22.75px] tracking-normal group-hover:text-white/60 transition-colors duration-500">
                {item.description}
              </p>
            </li>
          ))}
